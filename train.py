...
from swinTransformer.swin_model import swin_base_patch4_window12_384_in22k as create_model
from model import SwinTransformerFPN, CountRegressor, weights_normal_init
...
parser.add_argument("-dp", "--data_path", type=str, default='./data/DataSets/FSC147_384_V2/', help="Path to the "
                                                                                           "FSC147 dataset")
parser.add_argument('--weights', type=str, default='./weights/swin_base_patch4_window12_384_22k.pth',
...
origin_model = create_model().cuda()
assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
weights_dict = torch.load(args.weights, map_location='cuda:0')["model"]
origin_model.load_state_dict(weights_dict, strict=False)
origin_model.eval()

swinTrans = SwinTransformerFPN(origin_model, window_size=12)
swinTrans.cuda()
swinTrans.eval()
...
def train():
    ...
    for im_id in pbar:
        ...
        with torch.no_grad():
            # Freeze the backbone
            features = extract_features(swinTrans, image.unsqueeze(0), boxes.unsqueeze(0), MAPS, Scales)
...
def eval():
    ...
    for im_id in pbar:
        ...
        with torch.no_grad():
            output = regressor(extract_features(swinTrans, image.unsqueeze(0), boxes.unsqueeze(0), MAPS, Scales))
